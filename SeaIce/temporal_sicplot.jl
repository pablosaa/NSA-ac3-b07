...
datum = (12,2019) # (12,2019), (1,2020), (2,2020), (3,2020), (4,2020)) 
days = (31)
...
#let data = :SIC
    ...
    i_rv = 720 # for i_rv ∈ 1:nrow(winddir) #iday
        ...
        #for iti ∈ Tim
        iti=i_rv  ## argmin(abs.(winddir[!,:date].-RV[:time][i_rv]))
    ...
    # creating wind lines to plot
            ...
            GMT.basemap!(region=(LonLim..., LatLim...),
                 ...
                 rose = (outside=true, anchor=:TR, width=1., frame=false, offset=(0.5, 0.08), label=true) #(-0.5, -1.0), label=true)
	                 )
            ...
            GMT.plot!(nsa_lon, nsa_lat, marker=:star, markersize=0.2, mc=:red, show=1, fmt=:png) #, savefig=plot_name)
